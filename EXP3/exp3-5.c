...
int main(void)
{
	...
	uint8_t index = 0;
	uint8_t monthshift = 0;
	int i = 0;
	char uart_receive_char;
	char msg[7] = {'\0'};  // The message received
	...
	while (1)
	{
		...
		while(strcmp(msg, "\0") != 0 && strncmp(msg, month[index], 3) != 0)
		{
			index++;
		}
		if (strcmp(msg, "\0") != 0)
			monthshift = (msg[4] - '0') * 10 + (msg[5] - '0') * 1;
		
		if (msg[3] == '+')
			UARTStringPutNonBlocking(month[(index + monthshift) % 12]);
		if (msg[3] == '-')
			UARTStringPutNonBlocking(month[(index + 12 - monthshift % 12) % 12]);
