...
int main(void)
{
	...
	//char* msg = (char *)malloc(14);  // The message received
	char msg[14] = {'\0'};
	char classnum[] = "CLASSF1903202";
	char codenum[] = "CODE519021910411";  // message to be sent to UART
	...
	while (1)
	{
		...
		if (uart_receive_status)
		{
			...
			{
				//Read the next character from the UART and concantenate the string.
		...
		if (strcmp(msg, "AT+CLASS") == 0 || strcmp(msg, "at+class") == 0)
			UARTStringPut(classnum);
		
		if (strcmp(msg, "AT+STUDENTCODE") == 0 || strcmp(msg, "at+studentcode") == 0)
			UARTStringPut(codenum);
