...
volatile uint16_t systick_1s_counter = 0; // 1s clock counter
volatile uint8_t systick_1s_status = 0;  // 1s clock state

volatile uint8_t uart_receive_status = 0;

//self-increment clock counter define
bool minute_step = false;
bool hour_step = false;
uint32_t second = 0;
uint32_t minute = 0;
uint32_t hour = 12;

//msg 
char time[9] = {'\0'};   // Current time
...
int main(void)
{
	...
	while (1)
	{
		...
		{
			hour = (msg[3] - '0') * 10 + (msg[4] - '0') * 1;
			minute = (msg[6] - '0') * 10 + (msg[7] - '0') * 1;
			second = (msg[9] - '0') * 10 + (msg[10] - '0') * 1;
			Get_Time();
		}
		
		if (strncmp(msg, "INC", 3) == 0)
		{
			if (second + (msg[9] - '0') * 10 + (msg[10] - '0') * 1 > 59)
				minute += 1;
			second = (second + (msg[9] - '0') * 10 + (msg[10] - '0') * 1) % 60;
			if (minute + (msg[6] - '0') * 10 + (msg[7] - '0') * 1 > 59)
				hour += 1;
			minute = (minute + (msg[6] - '0') * 10 + (msg[7] - '0') * 1) % 60;
			hour = (hour + (msg[3] - '0') * 10 + (msg[4] - '0') * 1) % 24;
...
{
	if (systick_1s_counter == 0) //利用1ms的SysTick产生1s的定时器
	{
