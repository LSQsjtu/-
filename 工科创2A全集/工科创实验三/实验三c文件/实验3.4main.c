...
int main(void)
{
	...
	IntPrioritySet(FAULT_SYSTICK,0x3);		//Set INT_SYSTICK to highest priority
...
void SysTick_Handler(void)
{
	...
	//??2:??D1,??SW1??D2,?????
	GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,GPIO_PIN_1);	//??PN1(D1)
	while (GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0) == 0)//PJ0(USR_SW1) keep pressed
	{
		systick_100ms_status	= systick_10ms_status = 0;
		GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_0,GPIO_PIN_0); //??PN0(D2)
	}
...
void UART0_Handler(void)
{
  ...
  UARTIntClear(UART0_BASE, uart0_int_status);							//Clear the asserted interrupts
	...
	//??3:??D1,D2,??SW2??????
	GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_0|GPIO_PIN_1,0);	//??PN0,PN1
