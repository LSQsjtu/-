...
volatile uint16_t systick_10ms_couter=0, systick_100ms_couter=0,systick_5ms_couter=0; //10ms?100ms???
volatile uint8_t	systick_10ms_status=0, systick_100ms_status=0,systick_5ms_status=0; //10ms?100ms????
uint8_t key;//SW1~8
uint8_t num=1;
...
int main(void)
{
  ...
  IntMasterEnable();	//???
	...
	while (1)
	{
		switch(num)//change the frequency
	  {
		  case 1: I2C_FLASHTIME=500;break;
	    case 2: I2C_FLASHTIME=1000;break;
		  case 3: I2C_FLASHTIME=2000;break;
		  case 4: I2C_FLASHTIME=200;break;
	  }
		...
		if (systick_100ms_status) //100ms???
		{
			systick_100ms_status	= 0; //??100ms????
				...
				if (++i2c_flash_cnt		>= I2C_FLASHTIME/100)  //5*100ms=500ms
			  {
				  i2c_flash_cnt				= 0;
				  cnt = (cnt+1) % 8;
				  if(cnt==7) result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~((1<<cnt)+(1<<(cnt-7))));
				  else result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~((1<<cnt)+(1<<(cnt+1))));
	      }
...
void S800_GPIO_Init(void)
{
  ...
  GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, GPIO_PIN_0);			//Set PF0 as Output pin
	...
	GPIOIntTypeSet(GPIO_PORTJ_BASE, GPIO_PIN_0, GPIO_FALLING_EDGE); //GPIO interrupt for PJ0
...
void SysTick_Handler(void)
{
	...
	if (systick_10ms_couter	== 0) //??1ms?SysTick??10ms????
	{
		...
		systick_10ms_couter--;
	if (systick_5ms_couter	== 0) //??1ms?SysTick??5ms????
	{
		systick_5ms_couter	 = 10;//in fact it is another systick_10ms_couter
