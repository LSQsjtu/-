...
int main(void)
{
	...
	{
		//点数码管
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[num[i]]);				//write port 1（‘0’的码段值） 				
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,(uint8_t)(1<<i));		//write port 2（点亮第1个数码管，高点亮）
		...
		//点LED灯
		result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,0x0);	//点亮所有LED灯，低点亮

		//PF0闪烁
		GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_0, ~GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0));	// Turn over the PF0 
		SysCtlDelay(ui32SysClock/1500); //延时0.5s
