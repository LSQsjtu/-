  ...
  IntMasterEnable();	//开中断	
	...
	while (1)
	{
		...
		if (systick_100ms_status) //100ms定时到
		{
			...
			if (++i2c_flash_cnt		>= I2C_FLASHTIME/count)  //5*100ms=500ms
			{
				i2c_flash_cnt				= 0;
		...
		//数码管跑马灯
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[(cnt+control)%8+1]);	//write port 1 				
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,1<<(cnt+control)%8);				//write port 2
	}
...
void SysTick_Handler(void)
{
	...
	btn_state = GPIOPinRead(GPIO_PORTJ_AHB_BASE, GPIO_PIN_0);  //Recheck the state of the pin
	if (btn_state == 0x00)
	{
			btn_cnt = (btn_cnt + 1) % 4; 
	}
